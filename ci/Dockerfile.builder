...
# gavecoin_hash
RUN git clone https://github.com/gavecoin/gavecoin_hash
RUN cd gavecoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} gavecoin
RUN useradd -u ${USER_ID} -g gavecoin -s /bin/bash -m -d /gavecoin gavecoin
...
RUN mkdir /gavecoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /gavecoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /gavecoin-src
