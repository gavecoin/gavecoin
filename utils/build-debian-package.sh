...
PACKAGE_VERSION=$($SRC_PATH/src/gavecoind --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/gavecoind $DEB_BIN
cp $SRC_PATH/src/gavecoin-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/gavecoin-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/gavecoind.1 $DEB_MAN
cp $SRC_DOC/man/gavecoin-cli.1 $DEB_MAN
cp $SRC_DOC/man/gavecoin-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/gavecoind.bash-completion $DEB_CMP/gavecoind
cp $SRC_PATH/contrib/gavecoin-cli.bash-completion $DEB_CMP/gavecoin-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/gavecoind.1
gzip --best -n $DEB_MAN/gavecoin-cli.1
gzip --best -n $DEB_MAN/gavecoin-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/gavecoind $DEB_BIN/gavecoin-cli
