...
// which are part of the Gavecoin repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="gavecoin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /gavecoin-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "gavecoin-gavecoin/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from gavecoin-gavecoin/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'gavecoin-gavecoin/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from gavecoin-gavecoin/develop"
            }
          }

          if (hasCache) {
            sh "cd /gavecoin-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /gavecoin-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /gavecoin-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /gavecoin-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /gavecoin-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /gavecoin-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /gavecoin-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
