...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode", strprintf(_("Disable all Gavecoin specific functionality (Smartnodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-sporkaddr=<gavecoinaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/gavecoin/gavecoin>";
    const std::string URL_WEBSITE = "<https://gavecoin.org>";
...
/** Sanity checks
 *  Ensure that Gavecoin Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Gavecoin Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Gavecoin Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
	...
	GenerateGavecoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
    ...
    // lite mode disables all Gavecoin-specific functionality
    fLiteMode = gArgs.GetBoolArg("-litemode", false);
    LogPrintf("fLiteMode %d\n", fLiteMode);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, most Gavecoin-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/gavecoin/gavecoin/pull/1817 and https://github.com/gavecoin/gavecoin/pull/1743
    ...
    // ********************************************************* Step 10c: schedule Gavecoin-specific tasks
