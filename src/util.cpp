...
const char * const BITCOIN_CONF_FILENAME = "gavecoin.conf";
const char * const BITCOIN_PID_FILENAME = "gavecoind.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GavecoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GavecoinCore
    // Mac: ~/Library/Application Support/GavecoinCore
    // Unix: ~/.gavecoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GavecoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GavecoinCore";
#else
    // Unix
    return pathRet / ".gavecoincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty gavecoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override gavecoin.conf
