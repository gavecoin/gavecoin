...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total gavecoin balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            " - \"gavecoin\" activates all Gavecoin-specific categories at once;\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "gavecoin")
            + HelpExampleRpc("debug", "gavecoin+net")
        );
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given gavecoin address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The gavecoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The gavecoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Gavecoin address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are gavecoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) gavecoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The gavecoin address to use for the signature.\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Gavecoin features */
    { "gavecoin",               "mnsync",                 &mnsync,                 true,  {} },
    { "gavecoin",               "spork",                  &spork,                  true,  {"value"} },
