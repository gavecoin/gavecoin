...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 GAVECOIN irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on gavecoincentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in gavecoin.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("gavecoin.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in gavecoin.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Gavecoin features */
    { "gavecoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "gavecoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "gavecoin",               "gobject",                &gobject,                true,  {} },
    { "gavecoin",               "voteraw",                &voteraw,                true,  {} },
