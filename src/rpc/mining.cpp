...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated Gavecoin to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Gavecoin Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Gavecoin Core is downloading blocks...");
    ...
    if (sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)
        ...
        && CSuperblock::IsValidBlockHeight(chainActive.Height() + 1))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Gavecoin Core is syncing with network...");
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    int numCores = GenerateGavecoins(fGenerate, nGenProcLimit, Params());
