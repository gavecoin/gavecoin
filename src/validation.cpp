...
#if defined(NDEBUG)
# error "Gavecoin Core cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Gavecoin
...
void ThreadScriptCheck() {
    RenameThread("gavecoin-scriptch");
...
static int64_t nTimeGavecoinSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    /// GAVECOIN: Check superblock start
    ...
    // GAVECOIN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(GAVECOIN): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(GAVECOIN): spork is off, skipping transaction locking checks\n");
    }
    ...
    // GAVECOIN : MODIFIED TO CHECK SMARTNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(GAVECOIN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        return state.DoS(0, error("ConnectBlock(GAVECOIN): couldn't find smartnode or superblock payments"),
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(GAVECOIN): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeGavecoinSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Gavecoin specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeGavecoinSpecific * 0.000001);
